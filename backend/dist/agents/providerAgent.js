...
// ─── Agent Executor ───────────────────────────────────────────────────────────
async function run(rawInput, providerCategory) {
...
Provider Message: "${rawInput}"`;
    ...
    try {
        ...
        throw new error_middleware_1.ApiError(502, `Provider Agent LLM Connection Error: ${message}`);
    }
    try {
        ...
        throw new error_middleware_1.ApiError(502, `Provider Agent failed to parse LLM response.`);
    }
}
// ─── Runtime Type Guard ───────────────────────────────────────────────────────
function isProviderAgentOutput(value) {
    if (typeof value !== "object" || value === null)
        return false;
    const v = value;
    return (typeof v["replyText"] === "string" &&
