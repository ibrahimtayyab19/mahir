...
async function seed() {
    ...
    console.log(`✅  Connected: ${mongoose_1.default.connection.host}`);
    // ── Wipe all collections ──────────────────────────────────────────────────
    console.log("\n🗑️   Clearing all data...");
    await Promise.all([
        User_model_1.default.deleteMany({}),
        Provider_model_1.default.deleteMany({}),
        JobPost_model_1.default.deleteMany({}),
        Booking_model_1.default.deleteMany({}),
        AgentLog_model_1.default.deleteMany({}),
    ]);
    console.log("    ✓ Database cleared");
    const passwordHash = await bcryptjs_1.default.hash("123456", 10);
    // ── 1. Create Client Users ────────────────────────────────────────────────
    const clientUser = await User_model_1.default.create({
        name: "Admin Client",
    ...
    const demoClient = await User_model_1.default.create({
    ...
    for (const p of hardcodedProviders) {
        const providerUser = await User_model_1.default.create({
            name: p.name,
            email: p.email,
            passwordHash,
    ...
    for (let i = 0; i < 200; i++) {
        ...
        await JobPost_model_1.default.create({
